...
class TelephoneForm(forms.ModelForm):
    ...
    Bank_name_branch = forms.CharField(label='Bank Name and Branch*',widget=forms.Textarea(attrs={"placeholder":"Bank's Name and Branch","class"      : "new-class-name two",
    ...
    amount_words = forms.CharField(label='Amount in words*',widget=forms.Textarea(attrs={"placeholder":"Enter the amount in words","class"      : "new-class-name two",
                                    "id"         : "my-id-for-textarea","rows"       : 5,'cols'       : 19}))
                        
    
    amount       = forms.DecimalField(label='Amount*',initial=0.00)
    class Meta:
